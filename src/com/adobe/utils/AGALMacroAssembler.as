...
package com.adobe.utils
{
	...
	public class AGALMacroAssembler extends AGALMiniAssembler
	{
		...
		private function expandTexture(pos:int):int
		{
			...
			var s:String = "tex " + tokens[pos] + "," + tokens[openParen + 1] + "," + tokens[openParen + 3] + "<" + tokens.slice(openBracket + 1, closeBracket).join("") + ">;";
		...
		private function processTokens(pos:int, newline:int):int
		{
			...
			else if (types.length >= 4 && types.substr(pos, 4) == "i=i<" && tokens[pos + 2] == "tex")
			{
		...
		// Matrix multiply 3x3
		"macro mul3x3( vec, mat ) {" + "	m33 out, vec, mat; " + "}" + 
		// Matrix multiply 4x5
		"macro mul4x4( vec, mat ) {" + "	m44 out, vec, mat; " + "}"
	}
